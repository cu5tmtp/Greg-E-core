...
import net.cu5tmtp.GregECore.gregstuff.GregMachines.machines.endgame.SubatomicAntimatterSyntheticator;
...
public class SASRender extends DynamicRender<SubatomicAntimatterSyntheticator, SASRender> {

    public static final Codec<SASRender> CODEC = Codec.unit(new SASRender());
    public static final DynamicRenderType<SubatomicAntimatterSyntheticator, SASRender> TYPE = new DynamicRenderType<>(SASRender.CODEC);
    ...
    @Override
    public DynamicRenderType<SubatomicAntimatterSyntheticator, SASRender> getType() {
        return TYPE;
    }

    @Override
    public boolean shouldRender(SubatomicAntimatterSyntheticator machine, Vec3 cameraPos) {
        return machine.getRecipeLogic().isWorking();
    }

    @Override
    public void render(SubatomicAntimatterSyntheticator machine, float partialTick,
        ...
        float p1X = (up.getStepX() * 9.0F) + (right.getStepX() * 8.0F) + 0.5F;
        float p1Y = (up.getStepY() * 9.0F) + (right.getStepY() * 8.0F) + 0.5F;
        float p1Z = (up.getStepZ() * 9.0F) + (right.getStepZ() * 8.0F) + 0.5F;

        float p2X = (up.getStepX() * 9.0F) + (left.getStepX() * 8.0F) + 0.5F;
        float p2Y = (up.getStepY() * 9.0F) + (left.getStepY() * 8.0F) + 0.5F;
        float p2Z = (up.getStepZ() * 9.0F) + (left.getStepZ() * 8.0F) + 0.5F;

        renderOrbAndLightning(poseStack, translucentBuffer, p1X, p1Y, p1Z, fullBright, animationTick, 0);
        renderOrbAndLightning(poseStack, translucentBuffer, p2X, p2Y, p2Z, fullBright, animationTick, 100);
    }
    ...
    @Override
    public boolean shouldRenderOffScreen(SubatomicAntimatterSyntheticator machine) {
        return true;
    }
    ...
    @Override
    public AABB getRenderBoundingBox(SubatomicAntimatterSyntheticator machine) {
